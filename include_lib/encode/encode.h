...
{
	u16 sr;     ///<sample rate
	u16 br;     ///<mp2?????bitrate?adpcm???,??blockSize,????256/512/1024/2048,??2048?????2048
	u32 nch;
...
typedef struct _REC_FILE_INFO
{
    ...
    u32 enc_frame_cnt;
    u32 enc_time_cnt;
    u32 cut_head_ms;	//??0.1ms
    u32 cut_tail_ms;	//??0.1ms
}REC_FILE_INFO;
...
u32 get_enc_time(RECORD_OP_API * rec_op_api);   //????
u32 updata_enc_time(RECORD_OP_API * rec_op_api);//??????

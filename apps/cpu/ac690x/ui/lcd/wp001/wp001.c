...
static void lcd_hard_port_init(uint8 enable)
{
	...
	#if defined(_CHANGE_LCD_VOLTAGE1_)
		JL_LCDC->CON0 = 0x0004<<4;	//[6:4]:VLCDS 3.0V
	#elif defined(_CHANGE_LCD_VOLTAGE2_)
		JL_LCDC->CON0 = 0x0002<<4;	//[6:4]:VLCDS 2.8V
    #elif defined(_GL_LCD_VOLTAGE_)
		JL_LCDC->CON0 = 0x0005<<4;	//[6:4]:VLCDS 3.1V
    #elif defined(_WK5953_LCD_VOLTAGE_)
	...
	#if defined(_ENABLE_DISP_PIN_)
	    // 设置SEG	
		JL_LCDC->SEG_IOEN0 = 0xFFFC;
	#elif defined(_SUPPORT_PA9PA10_)
		// 设置SEG	
		JL_LCDC->SEG_IOEN0 = 0xF9FF;
	#else
		// 设置SEG	
		JL_LCDC->SEG_IOEN0 = 0xFFFF;
	#endif	
...
#define SET_COM_VAL(reg, val)			do{	disp_temp = (reg); 					\
								   		disp_temp &= 0x0600; 					\
										disp_temp |=(val&0xF9FF);				\
										(reg) = disp_temp;						\
									}while(0)
#else
#define SET_COM_VAL(reg, val)			do{	disp_temp = (reg); 					\
								   		disp_temp &= ~0xFFFF; 					\
										disp_temp |=(val&0xFFFF);				\
										(reg) = disp_temp;						\
									}while(0)
#endif

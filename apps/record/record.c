...
extern void rd_da_rec(short *buf);//录音使用
extern void rd_updata_rec(void);//录音使用
...
void rec_msg_deal(RECORD_OP_API **rec_api_p, int *msg)
{
    ...
    case MSG_REC_STOP:
		...
		//当录制fm或者aux时，录制完成会显示一下当前录音的文件编号
		#if !defined(_SUPPORT_RECORD_)
    		ui_disp.now_level = DISP_LEVEL_SETTING;
    		ui_disp.timer = 1500;
    		rec_num_disp(DISP_LEVEL_SETTING,rec_fname_cnt+1);//由于还没有更新，所以要主动加一
    	#endif
    	#if 0	//停止录音，不释放录音资源
    		stop_rec(*rec_api_p);

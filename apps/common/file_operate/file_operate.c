...
#if defined(_SPECIAL_PREV_FILE_)||defined(_ENABLE_RPT_DIR_BUG_)
#include "setup.h"
#endif
...
{
    //ew_printf("cmd11111111111111111111111111111111111 = %d\n",cmd);
    ...
    switch(cmd)
    {
        ...
        case FOP_FIND_SPEC_LGDEV:
                ...
                if(status<=0)
                {
					file_op_deg("---55555FOP_FIND_SPEC_LGDEV\n");
...
//                            printf_buf(tmpbp,VM_FILE_BK_LEN);
                ...
                {
					file_op_deg("---3333FOP_READ_BPINFO\n");
                    status=FILE_OP_ERR_LGDEV_NO_FIND;
                }
...
//                            printf_buf(tmpbp,VM_FILE_BK_LEN);
                ...
                {
					file_op_deg("---7777FOP_READ_FLACBPINFO\n");
                    status=FILE_OP_ERR_LGDEV_NO_FIND;
                }
...
static u32 file_sel_op(u32 cmd, FILE_OPERATE *fop, void *input, void *output)
{
    ...
    switch(cur_fop->fop_init->cur_play_mode)
	{
		case REPEAT_FOLDER:
	    	...
	    	ew_printf("first_file = %d\n",first_file);
	    	ew_printf("last_file = %d\n",last_file);
	    	setup.first_dir = first_file;
	    	setup.last_dir = last_file;
	    #endif

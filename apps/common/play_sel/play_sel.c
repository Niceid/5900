...
//    }
	...
	#if defined(SET_PLAY_TONE_VOL_SETTLED)
	set_sys_vol(TONE_VOL_SETTLED_NUM, TONE_VOL_SETTLED_NUM, FADE_ON);
	#else
...
__run_loop_psel:
        ...
        case SYS_EVENT_DEC_END:
            ...
            else
            {
                ...
                if(rpt)
                {
					printf("2222222222222play_sel_api\n");
                    file_cnt = 0;
                    OSTimeDly(psel->file_par->delay);
                    err = play_sel_api(mapi, psel, file_cnt);
                }

...
//    printf("music_file,cur_num= %d,total_num= %d \n",music_cur_filenum,music_cur_filetotal);
    ...
    if(music_info.file_name->lfn_cnt == 0)
    {
        tmp32 = strlen((void*)music_info.file_name->lfn);
        tmp8 = FRAME_TYPE_ANSI;
    }
    else
    {
        tmp32 = music_info.file_name->lfn_cnt;
        tmp8 = FRAME_TYPE_UTF16;
    }

    rcsp_db_printf("play_file's name: %d\n",tmp32);

    rcsp_put_buf((void*)music_info.file_name->lfn,tmp32);
    packet_frame_data(frame_p+cnt,NULL,"FILN",tmp8,(void*)music_info.file_name->lfn,tmp32);
    cnt++;

...
void rcsp_music_task_msg_deal_before(int *msg)
{
	...
	switch(msg[0])
	{
		case SYS_EVENT_PLAY_SEL_END:
			...
			if(music_info.tpath ==NULL)
			{
				music_info.tpath= malloc(128 + ALIGN_4BYTE(sizeof(LONG_FILE_NAME)) *2);
				if(music_info.tpath ==NULL)
				{
				...
				{
					music_info.file_name = (LONG_FILE_NAME*) (music_info.tpath + 128);
					music_info.dir_name = (LONG_FILE_NAME*) (music_info.tpath + 128 + ALIGN_4BYTE(sizeof(LONG_FILE_NAME)));
				}
			}
			else
			{
		...
		case SYS_EVENT_DEL_TASK:
		    ...
		    if(music_info.tpath != NULL)
   			{
        		free(music_info.tpath);
        		music_info.tpath = NULL;
   			}

...
LIGHT_ATTRIBUTE light_attr;      //灯属�?
u8 light_on_off = 1;             //灯开�?
u8 flicker_effect;               //灯闪烁模�?0:LED_FLASH_100MS  1:LED_FLASH_300MS  2:LED_FLASH_500MS
...
*Description: RGB灯控制函�?
*Arguments  : status:0 关闭   1 打开
              r_duty:红灯占空�? (1-0xFF)/0xFF
              g_duty:绿灯占空�? (1-0xFF)/0xFF
              b_duty:蓝灯占空�? (1-0xFF)/0xFF
              fade:1-淡入淡出�?-立即响应
*Returns    : NULL
*Note       :
*By         :
...
//        }
        ...
        set_brightness((255*light_parm_p->brightness)/100,LED_FADE_ON);  //亮度值转�?手机APP端发过来的亮度值是0-100�?
        light_on_off = 1;
        break;

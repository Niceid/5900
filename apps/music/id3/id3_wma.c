 ...
 * \note
 *******************************************************************************/
static inline uint16 wma_mini_num(uint16 a, uint16 b)
{
    if(b < a)
    {
        return b;
...
static char wma_check_tag(void)
{
    ...
    for (i = 0; i < EXTENDED_TYPE_CN; i++)
    {
        ...
        {
            if (memcmp(key_buf, tag_string_info[i].str_buf, (uint32) tag_string_info[i].str_len) == 0)
            {
                check_flag[i] = 0;
                break;
            }
...
uint8 get_id3_wma(id3_ctrl_t *id3_ctrl)
{
#if 0
	uint8 chk_content_flag = TRUE;
	uint8 chk_excontent_flag = TRUE;
    uint32 tmp_pos_file;
    uint32 obj_total_size, 
	uint32 object_size;
	
	id3_ctrl->id3_data
    if(memcmp(id3_ctrl->id3_data, asf_header_obj, 16) != 0)//compare header object
    {
        return FALSE;
    }
	id3_ctrl->id3_pos += 16;
	
	obj_total_size = *((uint32 *)(&id3_ctrl->id3_data[id3_ctrl->id3_pos]));	//计算header object 的ObjectSize
    id3_ctrl->id3_pos += 14;
	
    while(id3_pos_file < obj_total_size)
    {
        read_buf_data(key_buf, 24);
        object_size = *((uint32 *)(&id3_ctrl->id3_data[id3_ctrl->id3_pos + 16]));
        tmp_pos_file = 0;
		
        if(chk_content_flag == TRUE)
        {
            if(memcmp(&id3_ctrl->id3_data[id3_ctrl->id3_pos], content_description_obj, 16) == 0)
            {
        ...
        {
            if(memcmp(&id3_ctrl->id3_data[id3_ctrl->id3_pos], extended_content_description_obj, 16) == 0)
            {
                tmp_pos_file = id3_pos_file;

...
static int8 flac_check_tag(char *buf)
{
    ...
    for (i = 0; i < FLAC_ITEM_COUNT; i++)
    {
        if (check_flag[i] == 1)
        {
            cmp_ret1 = memcmp(buf, &item_key_tab[i], (uint32) item_key_len[i]);
            cmp_ret2 = memcmp(buf, &item_key_tab1[i], (uint32) item_key_len[i]);
...
uint8 get_id3_flac(id3_ctrl_t *id3_ctrl)
{
#if 0
	uint32 vorbis_size;
	bool result1 = FALSE;

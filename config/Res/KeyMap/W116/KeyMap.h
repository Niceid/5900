#ifndef _KEY_MAP_H_
#define _KEY_MAP_H_

//进入这里
#define ADC1_KEY1_TEST_VALUE				KEY_ADC_VALUE(22)
#define ADC1_KEY2_TEST_VALUE				KEY_ADC_VALUE(27)
#define ADC1_KEY3_TEST_VALUE				KEY_ADC_VALUE(31)

#define ADC2_KEY1_TEST_VALUE				KEY_ADC_VALUE(0)
#define ADC2_KEY2_TEST_VALUE				KEY_ADC_VALUE(3)
#define ADC2_KEY3_TEST_VALUE				KEY_ADC_VALUE(7)
#define ADC2_KEY4_TEST_VALUE				KEY_ADC_VALUE(12)
#define ADC2_KEY5_TEST_VALUE				KEY_ADC_VALUE(16)
#define ADC2_KEY6_TEST_VALUE				KEY_ADC_VALUE(22)
#define ADC2_KEY7_TEST_VALUE				KEY_ADC_VALUE(27)
#define ADC2_KEY8_TEST_VALUE				KEY_ADC_VALUE(31)
#define ADC2_KEY9_TEST_VALUE				KEY_ADC_VALUE(36)
#define ADC2_KEY10_TEST_VALUE				KEY_ADC_VALUE(44)

#if defined(SUPPORT_PARTY_CONTROL)
#define ADC3_KEY1_TEST_VALUE				KEY_ADC_VALUE(6)
#define ADC3_KEY2_TEST_VALUE				KEY_ADC_VALUE(11)
#define ADC3_KEY3_TEST_VALUE				KEY_ADC_VALUE(16)
#define ADC3_KEY4_TEST_VALUE				KEY_ADC_VALUE(23)
#define ADC3_KEY5_TEST_VALUE				KEY_ADC_VALUE(28)
#define ADC3_KEY6_TEST_VALUE				KEY_ADC_VALUE(34)
#define ADC3_KEY7_TEST_VALUE				KEY_ADC_VALUE(39)
#endif
/*-------------------------------------------------------------------*/

enum{
	KEY_VALUE_NULL,// 0
/*---------------------------------------------*/
	KEY_VALUE_MODE,
	KEY_VALUE_MUTE,
	KEY_VALUE_POWER_SEL,
/*---------------------------------------------*/
	KEY_VALUE_M1,
	KEY_VALUE_M2,// 15
	KEY_VALUE_M3,
	KEY_VALUE_M4,
	KEY_VALUE_M5,
	KEY_VALUE_M6,
	KEY_VALUE_PREV,
	KEY_VALUE_NEXT,// 20
	KEY_VALUE_BAND_AMS,
	KEY_VALUE_TIME,// 10
/*---------------------------------------------*/
#if defined(SUPPORT_PARTY_CONTROL)
	KEY_VALUE_ANSWER1,
	KEY_VALUE_MODE1,
	KEY_VALUE_REJECT1,
	KEY_VALUE_PREV1,
	KEY_VALUE_ADD1,
	KEY_VALUE_SUB1,
	KEY_VALUE_NEXT1,
#endif
/*---------------------------------------------*/
	KEY_VALUE_BAND,
	KEY_VALUE_AMS,
	KEY_VALUE_SEL,
	KEY_VALUE_EQ,
	KEY_VALUE_POWER,// 1
	KEY_VALUE_POWER_MODE,// 5
	KEY_VALUE_ST,// 3
	KEY_VALUE_TIME_MUTE,// 4
	KEY_VALUE_LOUD,
	KEY_VALUE_ADD,
	KEY_VALUE_SUB,
	KEY_VALUE_POWER_MUTE,
	KEY_VALUE_LOC,
	KEY_VALUE_NAVI_DOWN,
	KEY_VALUE_NAVI_UP,//26
	KEY_VALUE_EQ_LOUD,// 2
	KEY_VALUE_SCAN,
	KEY_VALUE_END,	// 30	
};

#define KEY_NAVI_UP				KEY_VALUE_NAVI_UP
#define KEY_NAVI_DOWN			KEY_VALUE_NAVI_DOWN

#define KEY_POWER_MODE			KEY_VALUE_POWER_MODE
#define KEY_POWER_MUTE		    KEY_VALUE_POWER_MUTE

#define KEY_TIME_MUTE		    KEY_VALUE_TIME_MUTE
#define KEY_POWER_SEL		    KEY_VALUE_POWER_SEL

#endif /* _KEY_MAP_DEMO_H_ */

